...
async function maybeUploadImage(supabase: Awaited<ReturnType<typeof createClient>>, formData: FormData): Promise<string | null | undefined> {
  return resolveImageInput(supabase, formData, "featuredImage", "gallery", "news")
}
...
export async function deleteNews(id: string): Promise<ActionResult> {
  const supabase = await createClient()
  const { data, error } = await supabase.from("margaret_news").update({ deleted_at: new Date().toISOString() }).eq("id", id).select("id")
