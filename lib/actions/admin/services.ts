...
async function maybeUploadImage(supabase: Awaited<ReturnType<typeof createClient>>, formData: FormData): Promise<string | null | undefined> {
  ...
  return uploadPublicFile(supabase, "department-media", "services", file)
}
...
export async function updateService(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  const { error } = await supabase.from("margaret_services").update(update as never).eq("id", parsed.data.id)
...
export async function deleteService(id: string): Promise<ActionResult> {
  const supabase = await createClient()
  const { error } = await supabase.from("margaret_services").update({ deleted_at: new Date().toISOString() }).eq("id", id)
