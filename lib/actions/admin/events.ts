...
export async function updateEvent(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  const { data, error } = await supabase.from("margaret_events").update(update as never).eq("id", parsed.data.id).select("id")
...
export async function deleteEvent(id: string): Promise<ActionResult> {
  const supabase = await createClient()
  const { data, error } = await supabase.from("margaret_events").update({ deleted_at: new Date().toISOString() }).eq("id", id).select("id")
