...
async function maybeUploadImage(supabase: Awaited<ReturnType<typeof createClient>>, formData: FormData): Promise<string | null | undefined> {
  return resolveImageInput(supabase, formData, "bannerImage", "department-media", "clinics")
}
...
export async function updateClinic(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  const { data, error } = await supabase.from("margaret_clinics").update(update as never).eq("id", parsed.data.id).select("id")
...
export async function deleteClinic(id: string): Promise<ActionResult> {
  const supabase = await createClient()
  const { data, error } = await supabase.from("margaret_clinics").update({ deleted_at: new Date().toISOString() }).eq("id", id).select("id")
