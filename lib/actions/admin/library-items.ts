...
export async function updateLibraryItem(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  const { data, error } = await supabase.from("margaret_library_items").update(update as never).eq("id", parsed.data.id).select("id")
...
export async function deleteLibraryItem(id: string): Promise<ActionResult> {
  const supabase = await createClient()
  const { data, error } = await supabase.from("margaret_library_items").update({ deleted_at: new Date().toISOString() }).eq("id", id).select("id")
